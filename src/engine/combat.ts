...
function down(c: Combatant, overkill: number): void {
  ...
  const dead = overkill >= 10 || character.race === 0
...
export class Combat {
  ...
  get partyStanding(): Combatant[] {
    return this.party.filter(alive)
  }

  get monstersStanding(): Combatant[] {
    return this.monsters.filter(alive)
  }

  get over(): boolean {
    return this.partyStanding.length === 0 || this.monstersStanding.length === 0
  }
  ...
  next(): string[] {
    this.round++
    const lines: string[] = []
    const order = [...this.partyStanding, ...this.monstersStanding]
      .map((c) => ({ c, initiative: this.random(9) + Math.floor(c.member.character.movement / 3) }))
      .sort((a, b) => b.initiative - a.initiative)
      .map((x) => x.c)

    for (const attacker of order) {
      if (!alive(attacker)) continue
      const foes = this.party.includes(attacker) ? this.monstersStanding : this.partyStanding
      if (foes.length === 0) break
      const target = foes[this.random(foes.length - 1)]!
      const attacks = Math.max(1, Math.round(attacker.member.character.attacks.count / 2))
      for (let i = 0; i < attacks && alive(target); i++) {
        const roll = this.random(19) + 1
        if (!hits(attacker.member.character, target.member.character, roll)) {
          lines.push(`${attacker.label} MISSES ${target.label}.`)
          continue
        }
        const damage = rollDamage(attacker.member.character, this.random)
...
export function labelMonsters(groups: readonly { member: Member; count: number }[]): Combatant[] {
  ...
  for (const group of groups) {
    for (let i = 0; i < group.count; i++) {
      ...
      seen.set(name, n)
      const character = { ...group.member.character, money: [...group.member.character.money], levels: [...group.member.character.levels] }
      combatants.push({ member: { character, items: group.member.items }, label: n === 1 ? name : `${name} ${n}` })
    }
  }
  return combatants
}
