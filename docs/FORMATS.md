...
The current map's id (the GEO block) is kept at 0x49C5: the Temple of Bane's script dispatches its events by whether it reads 31 (inside) or 24 (outside) there, so the engine writes it on every map load. The combat result at 0x6DC7 is tested three ways by the scripts — `< 1` (won), `= 129` (the party fled) and `> 128` (the party is down) — so a won fight writes 0, a rout (the last monsters ran) 1 — the city watch's script tests for exactly that — a flight 129 and a wipe 255; 128 itself would read as a win to the `> 128` test.
